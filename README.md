...
This configuration file will retreive a list of S2 products. Note the configuration file is specific to a platform becuae the downloader needs to get additional info about each product from a specific CDSE api end point. Therefore using an S2 configuration to download S1, and vice versa, will fail.
...
```
LUIGI_CONFIG_PATH=s2-list-luigi.cfg  PYTHONPATH='.' luigi --module cdse_downloader DownloadProductsFromList --productListFile=productlist.txt --local-scheduler
```
...
```
LUIGI_CONFIG_PATH=s2-england-luigi.cfg PYTHONPATH='.' luigi --module cdse_downloader GenerateReport --startDate=2019-07-06T0000 --endDate=2019-07-08T1200 --reportFile=test-report.csv --local-scheduler
```

This will generate a report file "test-report.csv" that lists the products avaialbe from CDSE that meet the specified parameters.
