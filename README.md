...
virtualenv -p python3 ./downloader.venv

source ./downloader.venv/bin/activate
...
AccessKeyId and SecretKey are the S3 access credentials for the Mundi s3 bucket
...
```
LUIGI_CONFIG_PATH=s2-england-luigi.cfg PYTHONPATH='.' luigi --module mundi_downloader DownloadProductsByArea --startDate=2019-07-06T0000 --endDate=2019-07-08T1200 --local-scheduler
```
...
### Luigi Arguments
    ...
    --satelliteCode
        Satellite code filter, currently either 'A' or 'B'

    Sentinel 1 arguments
    --s1RelativeOrbitNo
        The relative orbit number of the orbit you want to filter on i.e. 125
    --timelinessCategory
        The timeliness category for data we are downloading (Fast_24h|NRT_1h)

    Sentinel 2 arguments         
    --s2CloudCover      Maximum cloudcover as a percentage
    
    
 S1 relative orbit number, S2 cloud cover, product level (L1_ for S1, L1C for S2), S1 product type (e.g. GRD, SLC), and S1 timeliness category (Fast_24h or NRT_1h) :)    
...
## AWS s2-testing servers

Navigate to the install folder and activate the python environment.

```
cd /data2/mundi-downloader/mundi-downloader

source . ./downloader.venv/bin/activate
```

There is a list config file for s1 and one for s2

* s1-list-luigi.cfg
* s2-list-luigi.cfg

There are two config files for each geographic region

eg:
* s1-scotland-luigi.cfg
* s2-scotland-luigi.cfg
